...
class CausalMetric():
    ...
    def single_run(self, img_arr, img_tensor, explanation, verbose=0, save_to=None):
        ...
        for i in range(n_steps+1):
            ...
            startarr = cv2.cvtColor(start.numpy().reshape(48,48,3), cv2.COLOR_BGR2GRAY).reshape(1,48,48,1)
            pred = torch.from_numpy(self.model(startarr).numpy())
            pr, cl = torch.topk(pred, 2)
            if verbose == 2:
                print('{}: {:.3f}'.format(get_class_name_emotion(cl[0][0]), float(pr[0][0])))
                print('{}: {:.3f}'.format(get_class_name_emotion(cl[0][1]), float(pr[0][1])))
            scores[i] = pred[0, c]
            ...
            if verbose == 2 or (verbose == 1 and i == n_steps) or save_to:
                ...
                plt.imshow(start[0].reshape(48,48,3))
                plt.subplot(122)
                ...
                plt.title(title)
                plt.xlabel(ylabel)
                plt.ylabel(get_class_name_emotion(c))
                if save_to:
